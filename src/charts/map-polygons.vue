...
export default class VueComponent extends Vue {
  ...
  private async loadBoundaries() {
    const shapes = this.config.boundaries || this.config.shapes
    if (!shapes) return
  ...
  private calculateCentroids() {
    for (const feature of this.boundaries) {
      ...
      centroid.properties.id = feature.properties[this.config.join]
  ...
  private async loadDataset() {
    try {
      ...
      const datasetFilename = this.config.datasets[datasetId]
  ...
  private updateChart() {
    ...
    if (!this.config.join) {
      throw Error('Config requires "join" property to link datasets')
    }

    const datasetJoinCol = this.config.display.fill.join
    if (!datasetJoinCol) throw Error(`Cannot find column ${datasetJoinCol}`)

    const datasetValuesCol = this.config.display.fill.values
    if (!datasetValuesCol) throw Error(`Cannot find column ${datasetValuesCol}`)
    ...
    const idColumn = this.config.join
